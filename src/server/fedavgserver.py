...
class FedavgServer(BaseServer):
    ...
    def _sample_clients(self, exclude=[]):
        ...
        if exclude == []: 
            ...
            sampled_client_ids = sorted(random.sample(list(range(self.args.K)), num_sampled_clients))
        # Evaluation - randomly select unparticipated clients in amount of `eval_fraction` multiplied
            ...
            if num_unparticipated_clients == 0: # when C = 1, i.e., need to evaluate on all clients
                ...
                sampled_client_ids = sorted(list(range(self.args.K)))
            else:
    ...
    def _request(self, ids, eval=False, participated=False, getLoss=False):
        ...
        if getLoss:
            logger.info(
                f'[{self.args.algorithm.upper()}] [Round: {str(self.round).zfill(4)}] Request loss evaluation to {"all" if ids is None else len(ids)} clients!')
            results = []
            with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(ids), os.cpu_count() - 1)) as workhorse:
                for idx in TqdmToLogger(
                    ids,
                    logger=logger,
                    desc=f'[{self.args.algorithm.upper()}] [Round: {str(self.round).zfill(4)}] ...get losses... ',
                    total=len(ids)
                    ):
    ...
    def _get_highest_loss(self, client_losses):
        ...
        def __boltzmann_softmax(x, temperature):
        ...
        if self.args.sampling_type == 'stoch':
            ...
            distr = __boltzmann_softmax(losses, self.args.temp)
            ids = np.random.choice(ids, size=X, p=distr, replace=False)

        else:
            sort = sorted(client_losses.items(), key=lambda x: x[1])
            ids = list(map(lambda x: x[0], sort[-X:]))
            logger.info(
                f'[{self.args.algorithm.upper()}] [Round: {str(self.round).zfill(4)}] ...selected {len(ids)} clients for active sampling!')
    ...
    def update(self):
        ...
        if self.args.active_sampling:
            ...
            client_losses = self._request(selected_ids, eval=True, participated=True, getLoss=True)
        ...
        if self.loglosses:
            ...
            self._cleanup(selected_ids)
            self._broadcast_models(selected_ids)
            with open(f"./log/usable-logs/metrix/{self.args.exp_name}.txt", "a") as file:
                file.write(f"{str(client_losses)}\n")
